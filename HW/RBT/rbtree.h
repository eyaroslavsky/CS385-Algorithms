...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
		...
		while(x != NULL) {
        	...
        	Node<K, V> *scratch = new Node<K,V>(key, key_value.second);
        	scratch->parent = y;
        	if(y == NULL) {
        		root_ = scratch;
        	} else if(key > y->key()) {
...
private:
    ...
    void insert_fixup(Node<K, V> *z) {
    	...
    	while (z->parent != NULL && z->parent->color == RED) {
    		if (z->parent == z->parent->parent->left) {
    			scratch = z->parent->parent->right;
    			if (scratch != NULL && scratch->color == RED) {
    ...
    int diameter(Node<K, V> *node) const {
    	...
    	int leftdi = diameter(node->left);
    	int rightdi = diameter(node->right);
    	int righthi = height(node->right);
    	
    	return std::max(lefthi + righthi + 2, std::max(leftdi , rightdi));
    }
